...
def main():
    ...
    if choice == 'Input URL':
        st.subheader("Summarize text, extract text, or both! All you need is a URL.")
        ...
        if extract:
            try:
                ...
                #len_of_short_text = round(len(raw_text)/text_length)
                #st.write(summaryURL_result[:len_of_short_text])
                st.write(raw_text)
            except:
